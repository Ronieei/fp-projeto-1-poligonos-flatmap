...
public class PoligonosApp extends Application {
    ...
    protected List<String> tipoPoligonos(){
        ...
        return pontosPoligonos.stream()
                .flatMap(pontos -> {
                    int numeroDePontos = pontos.size();
                    String tipo = switch (numeroDePontos) {
                        case 3 -> "Triângulo";
                        case 4 -> "Quadrilátero";
                        case 5 -> "Pentágono";
                        case 6 -> "Hexágono";
                        default -> "Polígono com " + numeroDePontos + " lados";
                    };
    ...
    protected List<Double> perimetros() {
        return pontosPoligonos.stream()
                .flatMap(pontos -> {
                    Point perimetro = pontos.stream()
                            .reduce(
                                    new Point(pontos.get(pontos.size() - 1), pontos.get(0)),
                                    Point::new
                            );
                    return Stream.of(perimetro.distance());
                })
                .toList();
    }
